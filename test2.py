...
while True:
    try:
        ...
        def apply_trade_logic(df, SYMBOL):
            exchange = ccxt.bybit({
                'options': {
            ...
            for i in range(1, len(df)):
                open_positions = exchange.fetchOpenOrders()
                if not open_positions and executed_trade == False:
                ...
                if df.ordersignal[i]==2 and len(df.ordersignal)==0:   
                    ...
                    exchange.create_order(SYMBOL, 'market', 'sell', AMOUNT,params={'reduceOnly': True,'stopLossPrice': sl1, },)
                    exchange.create_order(SYMBOL, 'market', 'sell', AMOUNT,params={'reduceOnly': True,'takeProfitPrice': tp1,},)
                    executed_trade = True
                    ...
                    exchange.create_order(SYMBOL, 'market', 'buy', AMOUNT,params={'reduceOnly': True,'stopLossPrice': sl1, },)
                    exchange.create_order(SYMBOL, 'market', 'buy', AMOUNT,params={'reduceOnly': True,'takeProfitPrice': tp1,},)
    
        df = apply_trade_logic(df, SYMBOL)
